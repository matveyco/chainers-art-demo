...
export class DebugHud {
  ...
  update() {
    ...
    const rows = [
      ...
      ['Post', L ? (L.cf && L.cf.enabled ? [L.cf.rendering.samples > 1 ? `MSAA ${L.cf.rendering.samples}×` : 'no MSAA', L.cf.ssao.type !== pc.SSAOTYPE_NONE ? 'SSAO' : '', L.style === 'comic' ? 'ink' : '', 'bloom'].filter(Boolean).join(' · ') : 'off') : ''],
      ['Resolution', `${d.width}×${d.height} (${(d.maxPixelRatio || 1).toFixed(2)}× DPR cap)`],
      ['VRAM', `${(s.vram.totalUsed / 1048576).toFixed(1)} MB`],
      ['Character', `${fmt(this.info.charTris)} tris · ${this.info.clips} clips · ${this.info.bones} bones`],
      ['Weapons', `${fmt(this.info.weaponTris)} tris for all four`],
    ];
