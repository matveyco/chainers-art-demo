...
export class Sfx {
  ...
  play(name, vol = 1, param = 0) {
    ...
    switch (name) {
      ...
      // ---- brawl
      case 'gempop':
        this._noise(t, 0.1, 0.16 * v, 'bandpass', 1800, 3200, 1.4);
        [1568, 2093].forEach((f, i) => this._tone(t + i * 0.04, 0.1, 0.12 * v, f, f * 1.02, 'triangle'));
        break;
      case 'gem':                                                           // pickup: bright rising arpeggio
        [1319, 1661, 1976, 2637].forEach((f, i) => this._tone(t + i * 0.035, 0.12, 0.15 * v, f, f, 'triangle'));
        break;
      case 'gemdrop':
        [1976, 1661, 1319].forEach((f, i) => this._tone(t + i * 0.05, 0.12, 0.14 * v, f, f * 0.98, 'triangle'));
        this._noise(t, 0.2, 0.18 * v, 'bandpass', 2200, 900, 1.2);
        break;
      case 'superready':
        [784, 988, 1175, 1568].forEach((f, i) => this._tone(t + i * 0.06, 0.16, 0.16 * v, f, f, 'square'));
        break;
      case 'super':
        this._noise(t, 0.35, 0.3 * v, 'bandpass', 600, 2400, 0.8);
        this._tone(t, 0.3, 0.25 * v, 220, 880, 'sawtooth');
        break;
      case 'rally':
        [523, 659, 784, 1047].forEach((f, i) => this._tone(t + i * 0.05, 0.3, 0.16 * v, f, f, 'triangle'));
        this._noise(t, 0.4, 0.14 * v, 'highpass', 3000, 6000, 0.6);
        break;
      case 'wallbreak':
        this._noise(t, 0.5, 0.7 * v, 'lowpass', 1800, 120, 0.5, true);
        this._noise(t, 0.12, 0.4 * v, 'bandpass', 900, 300, 1.4);
        this._tone(t, 0.2, 0.4 * v, 120, 50, 'sine');
        break;
      case 'hurt':                                                          // the player takes a hit
        this._tone(t, 0.12, 0.35 * v, 320, 140, 'square');
        this._noise(t, 0.08, 0.25 * v, 'lowpass', 1500, 400, 0.8);
        break;
      case 'respawn':
        [392, 523, 659, 784].forEach((f, i) => this._tone(t + i * 0.05, 0.2, 0.13 * v, f, f, 'triangle'));
        break;
      case 'count': this._tone(t, 0.14, 0.3 * v, 660, 660, 'square'); break;
      case 'fight':
        this._tone(t, 0.5, 0.3 * v, 880, 880, 'square'); this._tone(t, 0.5, 0.22 * v, 1320, 1320, 'square');
        this._noise(t, 0.4, 0.2 * v, 'highpass', 2500, 5000, 0.6);
        break;
      case 'win':
        [523, 659, 784, 1047, 784, 1047, 1319].forEach((f, i) => this._tone(t + i * 0.11, 0.26, 0.2 * v, f, f, 'square'));
        break;
      case 'lose':
        [392, 370, 349, 262].forEach((f, i) => this._tone(t + i * 0.2, 0.34, 0.2 * v, f, f * 0.99, 'triangle'));
        break;
      case 'alarm': this._tone(t, 0.18, 0.2 * v, 988, 988, 'square'); this._tone(t + 0.2, 0.18, 0.2 * v, 784, 784, 'square'); break;
      case 'ui': this._tone(t, 0.05, 0.16 * v, 1200, 1500, 'triangle'); break;
      case 'select': this._tone(t, 0.06, 0.18 * v, 700, 1100, 'square'); this._tone(t + 0.06, 0.08, 0.14 * v, 1100, 1400, 'square'); break;
      default: break;
    }
  }
}
